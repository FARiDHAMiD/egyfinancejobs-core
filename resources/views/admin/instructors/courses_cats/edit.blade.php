...
<div class="container-fluid">
    <form action="{{ route('cats.update', $cat->id) }}" class="mb-5" method="post">
        @method('put')
        @csrf
                    ...
                    <div class="col-md-12">
                        <div class="mb-2">
                            ...
                            <select class="form-control" name="website">
                                <option value="0" {{$cat->website == 0 ? 'selected' : ''}}>Jobs Website</option>
                                <option value="1" {{$cat->website == 1 ? 'selected' : ''}}>Courses Website</option>
                            </select>
                            @error('question')
                    ...
                    <div class="col-md-12">
                        <div class="mb-2">
                            ...
                            <textarea name="question" class="form-control @error('question') is-invalid @enderror "
                                cols="30" rows="3">{{$cat->question}}</textarea>
                            @error('question')
                            <span role="alert" class="invalid-feedback">( {{ $message }} )</span>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-12">
                        <div class="mb-2">
                            ...
                            <textarea name="answer" class="form-control @error('answer') is-invalid @enderror "
                                cols="30" rows="3">{{$cat->answer}}</textarea>
                            @error('answer')
                            <span role="alert" class="invalid-feedback">( {{ $message }} )</span>
                            @enderror
                        </div>
