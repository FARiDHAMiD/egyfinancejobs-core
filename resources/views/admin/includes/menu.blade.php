        ...
        <li class="nav-item dropdown no-arrow mx-1">
            ...
            <div class="dropdown-list dropdown-menu dropdown-menu-right shadow animated--grow-in"
                ...
                <a class="dropdown-item d-flex align-items-center" id="{{$item->id}}">
                    ...
                    <div>
                        <div class="small text-gray-500">{{\Carbon\Carbon::parse($item->created_at)->diffForHumans()}}
                        </div>
                        <span class="font-weight-bold">{{$item->data['title']}}</span>
                        <button data-id="{{$item->id}}"
                            data-route="{{(str_contains($item->data['url'], '?'))? $item->data['url'].'&notify='.$item->id : $item->data['url'].'?notify='.$item->id}}"
                            class="btn btn-success-outline mark-as-read"><i class="fa fa-eye"></i> mark as read</span>
                    </div>
                </a>
                @endforeach
