    ...
    <div class="container-fluid">
            ...
            <div class="card-body">
                <form action="" method="get" id="search-form">
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <input type="text" class="form-control b-sharp" name="salary" value="{{@$search['salary']}}" placeholder="Salary">
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <select name="job_category" class="form-control" >
                                    ...
                                    @foreach ($job_categories as $job_category)
                                        <option value="{{ $job_category->id }}"
                                            {{ @$search['job_category'] == $job_category->id ? 'selected' : '' }}>
                                            {{ $job_category->name }}
                                        </option>
                                    @endforeach
                                </select>
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <div class="row">
                    <div class="col-6">
                        <h6 class="m-0 font-weight-bold text-primary">Job List ({{$jobs->total()}})</h6>
                    </div>
                        ...
                        <tbody>
                            @foreach ($jobs as $job)
                                <tr>
                                    ...
                                    <td> {{ empty($job->area) ? null : $job->area->name }}, {{ empty($job->city) ? null : $job->city->name }}, {{ $job->country->name }}</td>
                                    ...
                                    <td>
                                        ...
                                        <form method="post" class="d-inline" action="{{ route('jobs.destroy', $job->id) }}">
                                            @method('delete')
                                            @csrf
                                            <button onclick="return confirm('Are you sure you want to delete this? it will delete all applications under this jobs! ')"  class="btn btn-danger btn-sm" type="submit">
